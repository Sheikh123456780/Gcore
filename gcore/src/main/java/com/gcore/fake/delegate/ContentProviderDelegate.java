...
import com.gcore.fake.service.context.providers.SettingsProviderStub;
...
public class ContentProviderDelegate {
    ...
    public static void update(Object holder, String auth) {
        ...
        if (auth.equals("settings")) {
            bContentProvider = new SettingsProviderStub().wrapper(iInterface, GreenBoxCore.getHostPkg());
        } else {
            bContentProvider = new ContentProviderStub().wrapper(iInterface, GreenBoxCore.getHostPkg());
        }
        if (BuildCompat.isOreo()) {
    ...
    public static void init() {
        ...
        GreenBoxCore.getContext().getContentResolver().call(Uri.parse("content://settings"), "", null, null);
        Object activityThread = GreenBoxCore.mainThread();
        ...
        for (Object value : map.values()) {
            ...
            if (!sInjected.contains(providerName)) {
                ...
                BRActivityThreadProviderClientRecordP.get(value)._set_mProvider(new ContentProviderStub().wrapper(iInterface, GreenBoxCore.getHostPkg()));
