...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (GActivityThread.getAppConfig() == null) {
                GreenBoxCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = GreenBoxCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            if (!GActivityThread.currentActivityThread().isInit()) {
                GActivityThread.currentActivityThread().bindApplication(activityInfo.packageName, activityInfo.processName);
                return true;
            }

            int taskId = BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).getTaskForActivity(token, false);
            GreenBoxCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(GreenBoxCore.mainThread()).getLaunchingActivity(token);
                if (record != null) {
                    ...
                    clientRecordContext._set_activityInfo(activityInfo);
                    clientRecordContext._set_packageInfo(GActivityThread.currentActivityThread().getPackageInfo());
                } else {
                    LaunchActivityItemContext launchActivityItemContext = BRLaunchActivityItem.get(r);
    ...
    private boolean handleCreateService(Object data) {
        if (GActivityThread.getAppConfig() != null) {
            String appPackageName = GActivityThread.getAppPackageName();
            assert appPackageName != null;

            ServiceInfo serviceInfo = BRActivityThreadCreateServiceData.get(data).info();
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(GActivityThread.getAppPid())) && !serviceInfo.name.equals(ProxyManifest.getProxyJobService(GActivityThread.getAppPid()))) {
                Slog.d(TAG, "handleCreateService: " + data);
                Intent intent = new Intent();
                intent.setComponent(new ComponentName(appPackageName, serviceInfo.name));
                GreenBoxCore.getBActivityManager().startService(intent, null, false, GActivityThread.getUserId());
                return true;
            }
        }
        return false;
