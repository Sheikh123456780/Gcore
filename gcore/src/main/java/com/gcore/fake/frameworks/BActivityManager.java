...
import com.gcore.core.system.am.IBActivityManagerService;
...
public class BActivityManager extends BlackManager<IBActivityManagerService> {
    ...
    public void onActivityResumed(IBinder token) {
        try {
            if ("com.tencent.mm".equals(GActivityThread.getAppPackageName())) {
                Activity activityByToken = GActivityThread.getActivityByToken(token);
    ...
    public void getIntentSender(IBinder target, String packageName, int uid) {
        try {
            getService().getIntentSender(target, packageName, uid, GActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
    }
    ...
    public String getPackageForIntentSender(IBinder target) {
        try {
            return getService().getPackageForIntentSender(target, GActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return null;
    ...
    public int getUidForIntentSender(IBinder target) {
        try {
            return getService().getUidForIntentSender(target, GActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return -1;
