...
import com.gcore.core.system.pm.IBPackageManagerService;
...
public class BPackageManager extends BlackManager<IBPackageManagerService> {
    ...
    public Intent getLaunchIntentForPackage(String packageName, int userId) {
        ...
        List<ResolveInfo> ris = queryIntentActivities(intentToResolve, 0, intentToResolve.resolveTypeIfNeeded(GreenBoxCore.getContext().getContentResolver()), userId);

        if (ris == null || ris.isEmpty()) {
            intentToResolve.removeCategory(Intent.CATEGORY_INFO);
            intentToResolve.addCategory(Intent.CATEGORY_LAUNCHER);
            intentToResolve.setPackage(packageName);
            ris = queryIntentActivities(intentToResolve, 0, intentToResolve.resolveTypeIfNeeded(GreenBoxCore.getContext().getContentResolver()), userId);
        }
        if (ris == null || ris.isEmpty()) {
            return null;
        }
    ...
    public String[] getPackagesForUid(int uid) {
        try {
            return getService().getPackagesForUid(uid, GActivityThread.getUserId());
        } catch (RemoteException e) {
            e.printStackTrace();
        }
        return new String[]{};
