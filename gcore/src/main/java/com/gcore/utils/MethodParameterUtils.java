...
public class MethodParameterUtils {
    ...
    public static String replaceFirstAppPkg(Object[] args) {
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof String value) {
                if (GreenBoxCore.get().isInstalled(value, GActivityThread.getUserId())) {
                    args[i] = GreenBoxCore.getHostPkg();
                    return value;
                }
            }
        }
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i] instanceof Integer) {
                ...
                if (uid == GActivityThread.getBUid()) {
                    args[i] = GreenBoxCore.getHostUid();
                }
            }
        }
    }
    ...
    public static void replaceLastAppPkg(Object[] args) {
        ...
        if (index != -1) {
            ...
            if (GreenBoxCore.get().isInstalled(pkg, GActivityThread.getUserId())) {
                args[index] = GreenBoxCore.getHostPkg();
            }
        }
    }
    ...
    public static void replaceLastUserId(Object[] args) {
        ...
        if (index >= 0 && args[index] instanceof Integer) {
            args[index] = GreenBoxCore.getHostUserId();
        }
    }
