...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        GreenBoxCore greenBoxCore = GreenBoxCore.get();
        for (String packageName : list) {
            if (greenBoxCore.isInstalled(packageName, userId)) {
                continue;
            }
            try {
                GreenBoxCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException e) {
                continue;
            }
            InstallResult installResult = greenBoxCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        GreenBoxCore greenBoxCore = GreenBoxCore.get();
        for (String packageName : list) {
            greenBoxCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            GreenBoxCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return GreenBoxCore.get().isInstalled(GMS_PKG, userId);
    }
}