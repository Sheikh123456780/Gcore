...
public class BJobManagerService extends IBJobManagerService.Stub implements ISystemService {
    ...
    public JobInfo schedule(JobInfo info, int userId) {
        ...
        if (processRecord == null) {
            processRecord = BProcessManagerService.get().startProcessLocked(serviceInfo.packageName, serviceInfo.processName, userId, -1, Binder.getCallingPid());
            if (processRecord == null) {
    ...
    public JobInfo scheduleJob(ProcessRecord processRecord, JobInfo info, ServiceInfo serviceInfo) {
        ...
        BRJobInfo.get(info)._set_service(new ComponentName(GreenBoxCore.getHostPkg(), ProxyManifest.getProxyJobService(processRecord.bpid)));
        return info;
    }
    ...
    @Override
    public int cancel(String processName, int jobId, int userId) {
        return jobId;
    }
