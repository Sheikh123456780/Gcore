package com.gcore.core.system.location;
...
import com.gcore.entity.location.BLocationConfig;
...
public class BLocationManagerService extends IBLocationManagerService.Stub implements ISystemService {
    ...
    public BCell getCell(int userId, String pkg) {
        ...
        return switch (config.pattern) {
            case BLocationManager.OWN_MODE -> config.cell;
            case BLocationManager.GLOBAL_MODE -> mGlobalConfig.cell;
            default -> null;
        };
    }
    ...
    public List<BCell> getAllCell(int userId, String pkg) {
        BLocationConfig config = getOrCreateConfig(userId, pkg);
        return switch (config.pattern) {
            case BLocationManager.OWN_MODE -> config.allCell;
            case BLocationManager.GLOBAL_MODE -> mGlobalConfig.allCell;
            default -> null;
        };
    }
    ...
    public BLocation getLocation(int userId, String pkg) {
        BLocationConfig config = getOrCreateConfig(userId, pkg);
        return switch (config.pattern) {
            case BLocationManager.OWN_MODE -> config.location;
            case BLocationManager.GLOBAL_MODE -> mGlobalConfig.location;
            default -> null;
        };
    }
    ...
    private void addTask(IBinder locationListener) {
        mThreadPool.execute(() -> {
            ...
            while (locationListener.pingBinder()) {
                ...
                GreenBoxCore.get().getHandler().post(() -> BRILocationListener.get(iInterface).onLocationChanged(location.convert2SystemLocation()));
            }
        });
    }
