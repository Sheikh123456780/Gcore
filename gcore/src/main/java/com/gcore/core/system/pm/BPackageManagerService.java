...
import com.gcore.entity.pm.InstalledPackage;
import com.gcore.utils.AbiUtils;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public BPackageManagerService() {
        ...
        GreenBoxCore.getContext().registerReceiver(mPackageChangedHandler, filter);
    }
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, GreenBoxCore.getHostPkg())) {
            try {
                return GreenBoxCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(Intent intent, int flags, int userId) {
        final String resolvedType = intent.resolveTypeIfNeeded(GreenBoxCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, GreenBoxCore.getHostPkg())) {
            try {
                return GreenBoxCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = GreenBoxCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
                FileUtils.copyFile(inputStream, apkFile);
            } else {
                apkFile = new File(file);
            }

            PackageInfo packageArchiveInfo = GreenBoxCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (!support) {
                return result.installError(packageArchiveInfo.packageName, GreenBoxCore.is64Bit() ? "Not support 32bit app" : "Not support 64bit app");
            }
    ...
    private int updateFlags(int flags, int userId) {
        if ((flags & (PackageManager.MATCH_DIRECT_BOOT_AWARE | PackageManager.MATCH_DIRECT_BOOT_UNAWARE)) != 0) {
            return flags;
        }
