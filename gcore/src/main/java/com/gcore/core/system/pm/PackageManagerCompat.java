...
public class PackageManagerCompat {
    ...
    public static PackageInfo generatePackageInfo(BPackage p, int flags, long firstInstallTime, long lastUpdateTime, BPackageUserState state, int userId) {
        ...
        try {
            base = GreenBoxCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
        ...
        if (BuildCompat.isQ()) {
            if ((flags & PackageManager.GET_SIGNING_CERTIFICATES) != 0) {
                if (base == null) {
                    BRPackageParserSigningDetails.get(null)._set_signatures(p.mSigningDetails.signatures);
                    pi.signingInfo = BRSigningInfo.get()._new(null);
                } else {
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = GreenBoxCore.getPackageManager().getApplicationInfo(GreenBoxCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = GreenBoxCore.getPackageManager().getPackageArchiveInfo(sourceDir, 0).applicationInfo;
        }
