...
public class AppJobServiceDispatcher {
    ...
    JobService getJobService(int jobId) {
        synchronized (mJobRecords) {
            ...
            try {
                JobRecord record = GreenBoxCore.getBJobManager().queryJobRecord(GActivityThread.getAppProcessName(), jobId);
                record.mJobService = GActivityThread.currentActivityThread().createJobService(record.mServiceInfo);
