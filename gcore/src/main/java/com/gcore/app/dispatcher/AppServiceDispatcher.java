package com.gcore.app.dispatcher;
...
import com.gcore.entity.ServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = GreenBoxCore.get().getHandler();
    ...
    public void onStartCommand(Intent proxyIntent, int flags, int startId) {
        ...
        try {
            ...
            GreenBoxCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
        } catch (Throwable e) {
            e.printStackTrace();
        }
    }
    ...
    public void onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = GreenBoxCore.getBActivityManager().onServiceUnbind(proxyIntent, GActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    GreenBoxCore.getBActivityManager().onServiceDestroy(proxyIntent, GActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    GreenBoxCore.getBActivityManager().onServiceDestroy(intent, GActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
    ...
    private Service getOrCreateService(ProxyServiceRecord proxyServiceRecord) {
        ...
        Service service = GActivityThread.currentActivityThread().createService(serviceInfo, token);
        if (service == null) {
            return null;
        }
