...
public class LauncherActivity extends Activity {
    ...
    public static void launch(Intent intent, int userId) {
        ...
        splash.setClass(GreenBoxCore.getContext(), LauncherActivity.class);
        ...
        GreenBoxCore.getContext().startActivity(splash);
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        PackageInfo packageInfo = GreenBoxCore.getBPackageManager().getPackageInfo(packageName, 0, userId);
        if (packageInfo == null) {
            finish();
            return;
        }
        Drawable drawable = packageInfo.applicationInfo.loadIcon(GreenBoxCore.getPackageManager());
        setContentView(R.layout.activity_launcher);
        findViewById(R.id.iv_icon).setBackground(drawable);
        new Thread(() -> GreenBoxCore.getBActivityManager().startActivity(launchIntent, userId)).start();
    }
