...
import com.darkbox.fake.hook.IInjectHook;
...
public class HCallbackStub implements IInjectHook, Handler.Callback {
    ...
    private Handler getH() {
        Object currentActivityThread = VBoxCore.mainThread();
 ...
 //   @Override
    public boolean handleMessage2(@NonNull Message msg) {
        if (!mBeing.getAndSet(true)) {
            try {
                if (BuildCompat.isPie()) {
                    if (msg.what == BRActivityThreadH.get().EXECUTE_TRANSACTION()) {
    ...
    private Boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                VBoxCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = VBoxCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            VBoxCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityRecord);

            if (BuildCompat.isS()) {
                Object record = BRActivityThread.get(VBoxCore.mainThread()).getLaunchingActivity(token);
                if (record != null) {
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid())) && !serviceInfo.name.equals(ProxyManifest.getProxyJobService(BActivityThread.getAppPid()))) {
                ...
                VBoxCore.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;
