...
public class ProxyPendingRecord {
    ...
    public static void saveStub(Intent shadow, Intent target, int userId) {
        shadow.putExtra("_V_|_P_user_id_", userId);
        shadow.putExtra("_V_|_P_target_", target);
    }

    public static ProxyPendingRecord create(Intent intent) {
        int userId = intent.getIntExtra("_V_|_P_user_id_", 0);
        Intent target = intent.getParcelableExtra("_V_|_P_target_");
