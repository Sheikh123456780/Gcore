...
import com.darkbox.utils.auth.Auth;
...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        VBoxCore sVBoxCore = VBoxCore.get();
        for (String packageName : list) {
            if (sVBoxCore.isInstalled(packageName, userId)) {
                continue;
            }

            try {
                VBoxCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException ignored) {
                continue;
            }

            InstallResult installResult = sVBoxCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        VBoxCore sVBoxCore = VBoxCore.get();
        for (String packageName : list) {
            sVBoxCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            VBoxCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
        } catch (PackageManager.NameNotFoundException ignored) { }
        return false;
    }
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return VBoxCore.get().isInstalled(GMS_PKG, userId);
    }
}