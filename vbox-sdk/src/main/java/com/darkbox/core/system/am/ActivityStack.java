...
import com.darkbox.core.system.pm.PackageManagerCompat;
import com.darkbox.proxy.ProxyActivity;
import com.darkbox.proxy.ProxyManifest;
import com.darkbox.proxy.record.ProxyActivityRecord;
...
public class ActivityStack {
    ...
    public ActivityStack() {
        this.mAms = (ActivityManager) VBoxCore.getContext().getSystemService(Context.ACTIVITY_SERVICE);
    }
    ...
    private int startActivityInNewTaskLocked(int userId, Intent intent, ActivityInfo activityInfo, IBinder resultTo, int launchMode) {
        ...
        VBoxCore.getContext().startActivity(shadow);
        return 0;
    }
    ...
    private int realStartActivityLocked(IInterface appThread, Intent intent, String resolvedType, IBinder resultTo, String resultWho, int requestCode, int flags,Bundle options) {
        try {
            ...
            BRIActivityManager.get(BRActivityManagerNative.get().getDefault()).startActivity(appThread, VBoxCore.getHostPkg(), intent,resolvedType, resultTo, resultWho, requestCode, flags, null, options);
        } catch (Throwable e) {
            e.printStackTrace();
        }
        return 0;
    ...
    private Intent getStartStubActivityIntentInner(Intent intent, int vPID, ProxyActivityRecord target, ActivityInfo activityInfo) {
        ...
        try {
            Resources resources = PackageManagerCompat.getResources(VBoxCore.getContext(), activityInfo.applicationInfo);
            int id;
            ...
            if (windowIsTranslucent) {
                shadow.setComponent(new ComponentName(VBoxCore.getHostPkg(), ProxyManifest.TransparentProxyActivity(vPID)));
            } else {
                shadow.setComponent(new ComponentName(VBoxCore.getHostPkg(), ProxyManifest.getProxyActivity(vPID)));
            }
    ...
    public ComponentName getCallingActivity(IBinder token, int userId) {
        synchronized (mTasks) {
            ...
            return new ComponentName(VBoxCore.getHostPkg(), ProxyActivity.P0.class.getName());
        }
    }
