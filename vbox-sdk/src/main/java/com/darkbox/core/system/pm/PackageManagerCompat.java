...
//                    }
        ...
        try {
            base = VBoxCore.getContext().getPackageManager().getPackageInfo(p.packageName, flags);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = VBoxCore.getPackageManager().getApplicationInfo(VBoxCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        String sourceDir = p.baseCodePath;
        if (p.applicationInfo == null) {
            p.applicationInfo = VBoxCore.getPackageManager().getPackageArchiveInfo(sourceDir, 0).applicationInfo;
        }
...
//            ApplicationInfo base = VBoxCore.getContext().getApplicationInfo();
