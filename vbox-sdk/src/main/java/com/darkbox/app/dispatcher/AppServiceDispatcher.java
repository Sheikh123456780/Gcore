package com.darkbox.app.dispatcher;
...
import com.darkbox.entity.ServiceRecord;
import com.darkbox.entity.UnbindRecord;
import com.darkbox.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = VBoxCore.get().getHandler();
...
//        Log.d(TAG, "onStartCommand: " + component.toString());
        ...
        try {
            ...
            VBoxCore.getBActivityManager().onStartCommand(proxyIntent, stubRecord.mUserId);
            return i;
        } catch (Throwable e) {
            e.printStackTrace();
        }
...
//        Log.d(TAG, "onLowMemory");
    ...
    public boolean onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = VBoxCore.getBActivityManager().onServiceUnbind(proxyIntent, BActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                ...
                if (destroy) {
                    service.onDestroy();
                    VBoxCore.getBActivityManager().onServiceDestroy(proxyIntent, BActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    VBoxCore.getBActivityManager().onServiceDestroy(intent, BActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();
